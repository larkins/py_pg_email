...
def _load_providers_from_db() -> Set[str]:
	"""Load major provider domains from the database."""
	try:
		conn = get_db_connection()
		cursor = conn.cursor()
		cursor.execute('SELECT domain FROM major_providers')
		providers = {row['domain'] for row in cursor.fetchall()}
		cursor.close()
		conn.close()
		return providers
	except Exception as e:
		logger.warning(f"Failed to load major providers from DB, using empty set: {e}")
		return set()
...
class GreylistManager:
	...
	def __init__(
		...
		self._major_providers = _load_providers_from_db()
		
		logger.info(
			f"Greylist manager initialized: "
			f"delay={delay_minutes}min, whitelist={whitelist_days}days, "
			f"major_providers={len(self._major_providers)}"
		)
	
	def reload_providers(self):
		"""Reload major providers from database."""
		self._major_providers = _load_providers_from_db()
		logger.info(f"Reloaded major providers: {len(self._major_providers)} domains")
	
	def _is_major_provider(self, sender: str) -> bool:
		"""Check if sender is from a major email provider (including subdomains)."""
		try:
			domain = sender.split('@')[-1].lower()
			if domain in self._major_providers:
				return True
			for provider in self._major_providers:
	...
	def check_sender(
		...
		try:
			...
			# (across ALL recipients, not just current one)
			if is_major and not result:
				...
				if any_whitelisted:
					...
					cursor.close()
					conn.close()
					logger.info(f"Greylist: Auto-whitelisted {sender} -> {recipient} (any {sender_domain} sender already whitelisted)")
					return True, None
