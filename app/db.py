...
def ensure_email_copy_schema():
	...
	try:
		...
		conn.rollback()
		import logging
		logging.getLogger(__name__).warning('ensure_email_copy_schema: InsufficientPrivilege tolerated (schema was set up by a prior privileged session)')
...
def ensure_domains_table():
	...
	try:
		...
		conn.rollback()
		import logging
		logging.getLogger(__name__).warning('ensure_domains_table: InsufficientPrivilege tolerated (schema was set up by a prior privileged session)')
...
def seed_local_domains():
	...
	try:
		...
		conn.rollback()
		import logging
		logging.getLogger(__name__).warning('seed_local_domains: InsufficientPrivilege tolerated (schema was set up by a prior privileged session)')
